...
function draw_mandelbrot(canvas, mandelbrot) {
    ...
    // If max_iterations is provided then use it.  Otherwise calculate it from the zoom parameter like normal.
    if ( mandelbrot.max_iterations === undefined || isNaN(mandelbrot.max_iterations) || mandelbrot.max_iterations < 0 ) { 
        var max_iterations = (255 + (30 * Math.log2(mandelbrot.zoom)));
    } else {
        var max_iterations = mandelbrot.max_iterations;
    }
    ...
    for (var y = 0; y < img.height; y += 1) {
        ...
        for (var x = 0; x < img.width; x += 1) {
            ...
            let h = 4 / mandelbrot.zoom;
            let w = 4 / mandelbrot.zoom;

            // z = z^2 + c
            var cx = (mandelbrot.x - w / 2) + x * (w / img.width);
            var cy = (mandelbrot.y - h / 2) + y * (w / img.width);
            var zx = 0;
            var zy = 0;
            var in_set = true;
            var num_iterations = 1;
            for (; num_iterations <= max_iterations; num_iterations += 1) {
...
var mandelbrot = {x: -0.6999687500000003, y: -0.2901249999999999, zoom: 1024, max_iterations: 500 };
